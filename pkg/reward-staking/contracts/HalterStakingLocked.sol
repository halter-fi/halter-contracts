...
contract HalterStakingLocked is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint256 public constant rewardRateCoeff = 1000000000000000000;
    uint256 public decimalsDenominator;
    ...
    mapping(address => LockWithdraw[]) public withdrawPurgatory; // returns array of user's withdraws under vesting
    ...
    event WithdrawLockStarted(address staker, uint256 weekNumber, uint256 amount);
    ...
    function initialize(
        ...
        address _updater,
        address _emergency,
        uint256 _decimalsDenominator // set as 1 if reward token has 18 decimals. as 10 if 17, 10 ** 2 if 16 etc...
    ) external initializer {
        __AccessControl_init();
        _setupRole(TREASURY_ROLE, _treasury);
        _setupRole(UPDATER_ROLE, _updater);
        _setupRole(EMERGENCY_ROLE, _emergency);
    ...
    */
    function setWeekState(uint256 _weekNumber, uint256 _rewardRate) public onlyRole(UPDATER_ROLE) {
        ...
        weekInfo[_weekNumber].totalStakedAmount = stakeToken.balanceOf(address(this));
    ...
    */
    function stake(uint256 _amount, uint256 _weekNumber) public duringCorrectWeek(_weekNumber) nonReentrant {
    ...
    */
    function startWithdrawLock(uint256 _amount, uint256 _weekNumber)
        public
        ...
        nonReentrant
    {
        ...
        withdrawPurgatory[msg.sender].push(
            LockWithdraw({ amount: _amount, unlockTime: block.timestamp.add(withdrawUnlockTime) })
        );
        ...
        emit WithdrawLockStarted(msg.sender, _weekNumber, _amount);
    }

    function withdrawUnlockedTokens() public nonReentrant {
        uint256 reward;
        for (uint256 i = 0; i < withdrawPurgatory[msg.sender].length; i++) {
            if (withdrawPurgatory[msg.sender][i].unlockTime <= block.timestamp) {
                reward = reward.add(withdrawPurgatory[msg.sender][i].amount);
                withdrawPurgatory[msg.sender][i].amount = 0;
            }
    ...
    */
    function claimRewards(uint256 _weekNumber) public duringCorrectWeek(_weekNumber) nonReentrant updated(_weekNumber) {
        uint256 reward;
        uint256 vestedDirtyReward = _getVestedReward();
        uint256 vestedDirtyWithdraw = _getFalseVestedReward();
        vestedDirtyWithdraw = vestedDirtyWithdraw.add(_calculateVestedDirtyWithdraw(_weekNumber));
        vestedDirtyReward = vestedDirtyReward.add(_calculateVestedDirtyReward(_weekNumber));
    ...
    function viewWithdrawPurgatoryArray() public view returns (LockWithdraw[] memory) {
        LockWithdraw[] memory arr = new LockWithdraw[](withdrawPurgatory[msg.sender].length);
        for (uint256 i = 0; i < withdrawPurgatory[msg.sender].length; i++) {
            arr[i].amount = (withdrawPurgatory[msg.sender][i].amount);
            arr[i].unlockTime = (withdrawPurgatory[msg.sender][i].unlockTime);
        }
        return arr;
    }
        ...
        returns (uint256 totalVestedWithdraw)
    {
        for (uint256 k = (userWithdraw[msg.sender][_index].lastWeekCalculated.add(1)); k < _weekNumber; k++) {
            uint256 weeklyReward = secondsInAWeek.mul(weekInfo[k].rewardRate).div(decimalsDenominator);
            ...
            if (userWithdraw[msg.sender][i].weekNumber != _weekNumber) {
                ...
                if (userWithdraw[msg.sender][i].lastWeekCalculated > 0) {
                    ...
                    for (uint256 l = (week.add(1)); l < _weekNumber; l++) {
                        totalVestedWithdraw = totalVestedWithdraw.add(
                            secondsInAWeek.mul(weekInfo[l].rewardRate).div(decimalsDenominator)
                        );
                    }
                }
            }
        ...
        for (uint256 i = 0; i < userStake[msg.sender].length; i++) {
            if (userStake[msg.sender][i].weekNumber != _weekNumber) {
                ...
                if (userStake[msg.sender][i].lastWeekCalculated > 0) {
                    ...
                    uint256 vestedReward = weekInfo[week].rewardRate
                ...
                for (uint256 l = (week.add(1)); l < _weekNumber; l++) {
                    totalVestedReward = totalVestedReward.add(
                        weekInfo[l].rewardRate.mul(userStake[msg.sender][i].staked).div(rewardRateCoeff).div(
                            decimalsDenominator
                        )
                    );
                }
        ...
        returns (uint256 vestedDirtyWithdraw)
    {
        for (uint256 i = 0; i < _idx; i++) {
            if (userWithdraw[msg.sender][i].weekNumber != _weekNumber) {
                uint256 week = userWithdraw[msg.sender][i].weekNumber;
                uint256 dirtyWithdraw = userWithdraw[msg.sender][i]
                    .stakesWithdrawn
                    .mul(weekInfo[week].rewardRate)
                ...
                for (uint256 k = (week.add(1)); k <= _weekNumber; k++) {
                    dirtyWithdraw = dirtyWithdraw.add(
                        secondsInAWeek.mul(weekInfo[k].rewardRate).div(decimalsDenominator)
                    );
                }

                if (timeLeft > 0) {
                    vestedDirtyWithdraw = vestedDirtyWithdraw.add(
                        vestedDirtyWithdraw.div(rewardsVestingDuration.div(timePassed))
                    );
                } else {
                    vestedDirtyWithdraw = vestedDirtyWithdraw.add(vestedDirtyWithdraw);
                }
            }
        }
    }
    ...
    function _calcUnvestedStake(
        ...
        unvestedReward = unvestedReward.div(secondsInAWeek).div(rewardRateCoeff).div(decimalsDenominator);
    ...
    function _calcUnvestedWithdraw(
        ...
        unvestedWithdraw = unvestedWithdraw.div(secondsInAWeek).div(rewardRateCoeff).div(decimalsDenominator);
    ...
    function _calcVestedWithdraw(
        ...
        vestedWithdraw = weekInfo[_weekIndex].rewardRate
            .mul(_time)
            .mul(userWithdraw[msg.sender][_stakeIndex].stakesWithdrawn)
            .mul(_secondsPassed);
        vestedWithdraw = vestedWithdraw
        .div(secondsInAWeek)
        .div(rewardRateCoeff)
        .div(rewardsVestingDuration)
        .div(decimalsDenominator);
    }
    ...
    function _calcVestedStake(
        ...
        vestedReward = weekInfo[_weekIndex].rewardRate
        .mul(_time)
        .mul(userStake[msg.sender][_stakeIndex].staked)
        .mul(
            _secondsPassed
        );
        ...
        for (uint256 i = 0; i < userWithdraw[msg.sender].length; i++) {
            if (userWithdraw[msg.sender][i].weekNumber != _weekNumber) {
                ...
                for (l; l < _weekNumber; l++) {
                    if (l != _weekNumber) {
                        uint256 weeklyReward = weekInfo[l].rewardRate
                        .mul(userStake[msg.sender][i].staked)
                        .div(rewardRateCoeff)
                        .div(decimalsDenominator);
    ...
    function _calcStakesAgain(uint256 _index, uint256 _weekNumber) internal view returns (uint256 totalVestedReward) {
        for (uint256 k = (userStake[msg.sender][_index].lastWeekCalculated.add(1)); k < _weekNumber; k++) {
            ...
            uint256 weeklyReward = userStake[msg.sender][_index].staked
            .mul(weekInfo[k].rewardRate)
            .div(rewardRateCoeff)
            .div(decimalsDenominator);
            if (secondsPassed > rewardsVestingDuration) {
                totalVestedReward = totalVestedReward.add(weeklyReward);
    ...
    {
        for (uint256 k = (userWithdraw[msg.sender][_index].lastWeekCalculated.add(1)); k < _weekNumber; k++) {
            uint256 weeklyReward = secondsInAWeek
            .mul(weekInfo[k].rewardRate)
            .div(rewardRateCoeff)
            .div(decimalsDenominator);
            uint256 secondsPassed = _calculateVestedTime(k).add(secondsInAWeek);
    ...
    function _getVestedReward() internal returns (uint256 reward) {
        for (uint256 i = 0; i < rewardUnclaimed[msg.sender].length; i++) {
            if (rewardUnclaimed[msg.sender][i].unlockTime <= block.timestamp) {
                ...
                uint256 currentReward = rewardUnclaimed[msg.sender][i].amount
    ...
    function _viewVestedReward() internal view returns (uint256 reward) {
        for (uint256 i = 0; i < rewardUnclaimed[msg.sender].length; i++) {
            if (rewardUnclaimed[msg.sender][i].unlockTime <= block.timestamp) {
                ...
                reward = reward.add(
                    rewardUnclaimed[msg.sender][i].amount
                        .mul(block.timestamp.sub(rewardUnclaimed[msg.sender][i].recordTime))
                        .div(rewardUnclaimed[msg.sender][i].unlockTime.sub(rewardUnclaimed[msg.sender][i].recordTime))
                );
            }
