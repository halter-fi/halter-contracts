...
describe('Halter Staking First Withdraw Locked Test', function () {
  ...
  context('Testing viewVestedRewards method with a single stake: ', function () {
      it('Should calculate rewards correctly after 1 withdraw', async function () {
        await advanceTime(week-20);
        ...
        expect(await this.halterStaking.connect(this.alice.address).viewVestedRewards('2')).to.be.at.most(
            BN.from('2382211169102682652')
          );
          expect(await this.halterStaking.connect(this.alice.address).viewVestedRewards('2')).to.be.at.least(
            BN.from('2381611169102682652')
          );
        await advanceTime(302996);
        await this.halterStaking.connect(this.alice).claimRewards('3');

        expect(await this.rewardToken.balanceOf(this.alice.address)).to.be.at.most(BN.from('3043333333333330000'));
        expect(await this.rewardToken.balanceOf(this.alice.address)).to.be.at.least(BN.from('3023333333333330000'));
      });
  });
});
