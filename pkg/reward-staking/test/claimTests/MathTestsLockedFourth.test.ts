...
describe('Halter Staking Fourth Claim Locked Test', function () {
    ...
    context('Testing claimRewards method with 2 stakes and 3 claims', function () {
        it('Should calculate view rewards correctly', async function () {
            ...
            expect(await this.halterStaking.connect(this.alice).viewVestedRewards('3')).to.be.at.least(BN.from('1205455555555560000'));
            expect(await this.halterStaking.connect(this.alice).viewVestedRewards('3')).to.be.at.most(BN.from('1205655555555560000'));

            await this.halterStaking.connect(this.alice).claimRewards('3');

            expect(await this.rewardToken.balanceOf(this.alice.address)).to.be.at.most(BN.from('2566866666666670000'));
